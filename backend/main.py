...
---
التعليمات الصارمة (STRICT RULES):
1. **اللغة (LANGUAGE)**: 
   - اكتب باللغة العربية الفصحى فقط. 
   - يمنع منعاً باتاً استخدام أحرف صينية (مثل "毫克", "服用") أو إنجليزية.
   - إذا أردت كتابة "ملجم" أو "مليجرام"، اكتبها بالعربي (ملجم) ولا تستخدم (mg) أو (毫克).
2. **الجرعة**: يجب أن تكون محددة بالأرقام (مثال: "5 مل" أو "قرص واحد"). لا تكتب "على فترات" بدون تحديد. إذا لم تذكر النشرة الجرعة، اكتب: "يرجى استشارة الطبيب لتحديد الجرعة".
3. **عدم التأليف**: لا تخترع أدوية غير موجودة في القائمة أعلاه.
...
- [اسم الدواء]: [الكمية (مل/قرص)] كل [عدد] ساعات (حسب النشرة).

💰 4. الأسعار:
- [اسم الدواء]: [السعر] ريال

⚠️ تحذيرات هامة: [تحذيرات من النشرة]"""
